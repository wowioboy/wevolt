function setPaymentInfo(isChecked)
{
	with (window.document.frmCheckout) {
		if (isChecked) {
			...
			document.getElementById('txtPaymentState').options[document.getElementById('txtShippingState').selectedIndex].selected = true;			
...
function checkShippingAndPaymentInfo()
{
	with (window.document.frmCheckout) {
		if (isEmpty(txtShippingFirstName, 'Enter first name')) {
			...
			return false;
		} else if (isEmpty(txtShippingEmail, 'Enter the shipping email address')) {
			return false;
			...
			return false;
		} else if ((optCod.checked == true) && (isEmpty(txtCCNumber, 'Enter the Payment credit card number'))) {
			return false;
		} else {
			return true;
		}
