...
$optionlist = $extracttext->extract($text,'<select name="url" onchange="if(this.options[this.selectedIndex].value != -1){ window.location = this.options[this.selectedIndex].value; }">',"</select>");

if (substr($optionlist,0,7) != '<option') {
	$optionlist = $extracttext->extract($text,'<select id="search1" name="url" onchange="if(this.options[this.selectedIndex].value != -1){ window.location = this.options[this.selectedIndex].value; }">
',"</select>");
}

if (substr($optionlist,0,7) != '<option') {
	 $optionlist = $extracttext->extract($text,'<select id="dropdown" name="url" onchange="if(this.options[this.selectedIndex].value != -1){ window.location = this.options[this.selectedIndex].value; }">',"</select>");
}
...
<? if ((!isset($_POST['submitlist'])) && (!isset($_POST['txtUrl']))) {?>
Please Enter the full URL to the first page of your comic on Smack Jeeves<br/>NOTE: This page must have the Arvhice Drop Down box visible
<form name="PageForm" method="post" action="extract_smackjeeves_working.php">
