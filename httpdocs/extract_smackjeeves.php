...
$UrlArray1 = explode('http://',$ImportUrl);
...
if(!is_dir("imported/temp/".$randDirectory."/thumbs")) mkdir("imported/temp/".$randDirectory."/thumbs", 0755); 
	foreach ($PageArray as $page) {
		$url = 'http://'.$UserString.'.smackjeeves.com'.$page;
	...
	// Regex that extracts the images (full tag)
		...
		foreach ($images_url_array as $Image) {
		 	 $UploadedFound = strpos($Image, "/images/uploaded/comics/");
			 $ServerFound = strpos($Image, "www.smackjeeves.com");
			if (($UploadedFound == true) && ($ServerFound  == true)){
				if (strlen($Count) == 2) {
...
<form name="PageForm" method="post" action="/comic/import/smackjeeves/<? echo $_GET['id'];?>/">
...
if ($SelectStart != '') {
	$Found = 1;
	$SelectText = '<select id="search1" name="url" onchange="if(this.options[this.selectedIndex].value != -1){ window.location = this.options[this.selectedIndex].value; }">';
}

//TEST AGAIN
if ($Found == 0){
$SelectStart = strpos($text, '<select id="dropdown" name="url" onchange="if(this.options[this.selectedIndex].value != -1){ window.location = this.options[this.selectedIndex].value; }">');
	if ($SelectStart != '') {
		$Found = 1;
		$SelectText = '<select id="dropdown" name="url" onchange="if(this.options[this.selectedIndex].value != -1){ window.location = this.options[this.selectedIndex].value; }">';
	}
}

if ($Found == 0){
$SelectStart = strpos($text, '<select name="url" onchange="if(this.options[this.selectedIndex].value != -1){ window.location = this.options[this.selectedIndex].value; }">');
	if ($SelectStart != '') {
		$Found = 1;
		$SelectText = '<select name="url" onchange="if(this.options[this.selectedIndex].value != -1){ window.location = this.options[this.selectedIndex].value; }">';
	}
}
...
if ($Found == 1) {
	...
	$text = substr($text, $SelectStart,($SelectStop+9));
 	$optionlist = $extracttext->extract($text,$SelectText,"</select>");
}
