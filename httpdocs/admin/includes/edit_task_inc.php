...
while ($i < $ProjectGroupCount) {
		...
		//print "MY USER COUNT =" . $GroupUsercount."<br/><br/>"; 
		$k=0;
		//print "I AM GOING TO LOOP THROUGH THE USERS IN GROUP '$GroupID' <br/><br/>";
		while ($k < $GroupUsercount) {
			$UserID = $GroupUsersArray[$k];
			//print "MY CURRENT USERID IN THE GROUP ARRAY= " . $UserID."<br/>";
			if ($GroupUsercount == 1) {
				$UserID = $GroupUsers;
			}
			//SELECT ALL THE DATA ON THE CURRENT GROUP USER
			...
			//print "THE NAME OF THE USER I'M TESTING IS $TestName<br/>"; 
			if ($Addedsize == 0) {
				$userString .= "<OPTION VALUE='".$UserInfo->UserId."'";
				//print "THERE ARE NO USERS ADDED YET, SO I WILL PUT $TestName WIN THE SELECT BOX<br/><br/>";
				$x=0;
				//print "NOW I WILL LOOP THROUG THE USERS CURRENTLY ASSIGNED TO THE TASK AND CHECK TO SEE IF I NEED SELECTED<br/>";
					while ($x < $TaskUsersCount) {
						//print "CURRENT TASK USER ID = " . $TaskUsersArray[$x]."<br/>";
						//print "THE ID I'M LOOKING FOR  = " .$UserID."<br/>";
						if (intval($TaskUsersArray[$x]) == $UserID) {
							$userString .= " selected ";
							//print "SHOULD BE ADDING SELECTED<br/><br/>";
							$x =$TaskUsersCount;
						}
						$x++;
					}
				$userString .= ">".$UserInfo->Username."</OPTION>";
				//print "FINISHED ADDING THE USER WHEN ADDED = 0 <br/><br/>";
			} else {
			 //print "STARTLING USER LOOP CHECK -----------------------<br/>";
				while ($j < $Addedsize) {
				//print "MY TEST ID = " .$UserID."<br/>";
				//print "MY TEST ADDED USERS ID = " .$AddedUsers[$j]."<br/>";
					if ($UserID == $AddedUsers[$j]) {
						$j = $Addedsize;
						$DontAdd = 1;
				...
				//	print $TestName.'WAS ADDED <br/><br/>';
					$userString .= "<OPTION VALUE='".$UserInfo->UserId."'";
						...
						while ($x < $TaskUsersCount) {
							if (intval($TaskUsersArray[$x]) == $UserID) {
								$userString .= " selected ";
								//print "SHOULD BE ADDING SELECTED<br/>";
								$x =$TaskUsersCount;
							}
						$x++;
						}
