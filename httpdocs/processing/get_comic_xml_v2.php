...
while ($Comic = $DB->FetchNextObject()) {
	if ($Count == 0) {
		...
		if ($Comic->ReaderBackground == '')
			$ReaderBackground = 'http://www.w3volt.com/images/reader_bg.jpg';
		else
			$ReaderBackground = $Comic->ReaderBackground;
			
		if ($Comic->TocImg == '')
			$TOCImg = 'http://www.w3volt.com/images/comic_toc_bkgrd.jpg';
		else
			$TOCImg =$Comic->TocImg;
		
		if ($Comic->ReaderCover == '')
			$CoverImage = 'http://www.panelflow.com'.$Comic->cover;
		else
			$CoverImage =$Comic->ReaderCover;	
		
		$XML = '<data bg="'.$ReaderBackground.'" bg_mp3="'.$bgMP3.'" id="'.$Content.'" p="'.$Pages.'">';
		
		$XML .= '<!--buttons>ALL|PAGE_SPREAD|ZOOM|FULLSCREEN|PRINT|VOLUME|SHARE|HELP|BUYNOW|LOGO</buttons-->
	<buttons>PAGE_SPREAD|ZOOM|FULLSCREEN|PRINT|VOLUME|SHARE|HELP|BUYNOW|LOGO</buttons>
				...
				if ($Episode == $EpisodeCount) {
					$InEpisode = 1;	
						...
						if (($UserSettingsArray->ProInvite > 0) || ($UserSettingsArray->IsPro > 0)) {
							$XML .= '<asset>http://www.panelflow.com/comics/'.$ComicDir.'/'.$Content.'/images/pages/'.$Comic->Filename.'</asset>
