	...
	if (($_REQUEST['a'] == 'edit') || ($_REQUEST['task'] == 'edit'))
		...
		if (($task == 'new')||($task == 'add')) {
			...
			$output .= $query.'<br/>';	
					...
					$query = "INSERT into pf_forum_boards (UserID, CatID, Title, Description, Position, CreatedDate, PrivacySetting,EventBoard,EventID) values ('".$_SESSION['userid']."', $CatID,'Event - $Title','General Discussion board for ".$_SESSION['username']."s event',1,'$CreatedDate','$Privacy',1,'$ModuleID')";
					$DB->execute($query);
					$output .= $query.'<br/>';	
			...
			if ((($EventType == 'event') || ($EventType == 'promotion')) &&($Privacy != 'private'))
				$ouput .= insertUpdate($EventType, 'created', $EncCalID, 'user', $UserID,'http://www.wevolt.com/view_event.php?action=view&id='.$CalID);
