...
$query = "SELECT * from pf_forum_topics where ID='".$_GET['topic']."'";
$ContentArray =  $DB->queryUniqueObject($query);

if ($_GET['msg'] == '') {
	if ($_SESSION['userid'] != $ContentArray->PosterID){?>
    <script type="text/javascript">
	
	document.location.href='/forum/index.php?<? echo $ForumType;?>=<? echo $TargetName;?>&a=read&topic=<? echo $_GET['topic'];?>';
	</script>
    <? }
	
} else {
	unset($ContentArray);
	$query = "SELECT * from pf_forum_messages where TopicID='".$_GET['topic']."' and ID='".$_GET['msg']."'";
	$ContentArray =  $DB->queryUniqueObject($query);
...
<form action="index.php?<? echo $ForumType;?>=<? echo $TargetName;?>&a=edit_topic" method="post">
<div align="center">
...
<input type='image' style="border:none;background:none;" src="http://www.wevolt.com/images/forums/cancelsave/save_off.png">&nbsp;&nbsp;
    <img src="http://www.wevolt.com/images/forums/cancelsave/cancel_off.png" onclick="window.location='index.php?<? echo $ForumType;?>=<? echo $TargetName;?>&a=read&topic=<? echo $_GET['topic'];?>';" class="navbuttons">
</div>	
...
<input type="text" style=" <? if ($_SESSION['IsPro'] == 1) {?>width:550px;<? } else {?>width:98%;<? }?>"  name="txtTitle" value="<? echo $ContentArray->Subject;?>"/>
<div class="spacer"></div>
You can enter just text, or paste HTML code by clicking the [html] button
<textarea name="content" id="content" style=" <? if ($_SESSION['IsPro'] == 1) {?>width:550px;<? } else {?>width:98%;<? }?> height:600px;"><? echo $ContentArray->Message;?></textarea>
<div class="spacer"></div>
<?php
/*
...
<input type="hidden" name="txtMessage" value="<? echo $_GET['msg'];?>" />
...
<? if ($_GET['msg'] == ''){?>
<div align="left" class="blue_med_header" >TOPIC SETTINGS</div> 
<hr/>
<div class="sender_name">Is Sticky</div>
<div class="messageinfo">
...
<div class="spacer"></div>
<div class="sender_name">Lock Topic</div>
...
<div class="spacer"></div>
<div class="sender_name">Privacy Setting</div>
...
<div class="spacer"></div>
<? } else {?>
<div align="left" class="blue_med_header" >MESSAGE SETTINGS</div> 
<hr/>
<div class="sender_name">Notify on Replies</div>
<input type="radio" value="1" name="txtNotify" <? if (($ContentArray->NotifyOnReply == 1) || ($ContentArray->NotifyOnReply == '')) echo 'checked';?> /> Yes &nbsp;&nbsp;<input type="radio" value="1" name="txtNotify" <? if ($ContentArray->NotifyOnReply == 0) echo 'checked';?> /> No
<div class="spacer"></div>
