...
<?php 
  function GenerateImage($WAPProfile, $ID, $StreamIt)
  {
	...
	$ItemDB->close();
	//print $Filename;
	$gif = file_get_contents($Filename) or die('Could not grab the file');
   ...
   // $ext = $Content->findexts(basename($FileName));
    ...
    // Get new dimensions
	//print "temp/".$Filename;
    list($width_orig, $height_orig) = getimagesize("temp/".$Filename);
   // print "File width = " .$width_orig;
	// print "File height = " .$height_orig;
    ...
    $image = imagecreatefromjpeg("temp/".$Filename);
