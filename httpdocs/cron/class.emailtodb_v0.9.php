...
class EMAIL_TO_DB {
 ...
 var $file_path = '/var/www/vhosts/wevolt.com/httpdocs/email/attachments/'; #Where to write file attachments to /var/www/files/email/
...
var $this_file_name = 'grabemail.php'; #If mode "html" 
...
function randomPrefix($length) {
 ...
 function connect($host, $port, $login, $pass, $randName){
  ...
  */
  ...
  function save_files($filename, $part){
    ...
    $fp=fopen($this->filestore.$this->randName.'.'.$ext,"w+");
    fwrite($fp,$part);
    fclose($fp);
	chmod($this->filestore.$this->randName.'.'.$ext,0777);
...
$ext = substr(strrchr($filename, "."), 1);
$filename = $this->filestore.$this->randName.'.'.$ext;
