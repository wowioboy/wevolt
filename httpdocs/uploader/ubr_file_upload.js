...
document.getElementById('upload_slots').style.display = 'none';
...
function addUploadSlot(num){
	if(upload_range < max_upload_slots){
		if(num == upload_range){
			...
			dv.innerHTML = '<input class="ubrUploadSlot" type="file" name="upfile_' + upload_range + '" size="30" onChange="addUploadSlot('+(upload_range + 1)+')" onKeypress="return handleKey(event)">';
			up.appendChild(dv);
			upload_range++;
			up = null;
			dv = null;
