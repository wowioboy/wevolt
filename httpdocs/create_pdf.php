...
function deleteimage(value) {
	...
	//alert(NewIDString);
		...
		document.PageForm.action='/create/pdf/?comicid=<? echo $_GET['comicid'];?>';
	  ...
	  $comicString = "<table width='99%' cellspacing='0' cellpadding='0' border='0'><tr>";
	   for ($i=0; $i< $nRows; $i++){
		...
		$UpdateDay = substr($row['PagesUpdated'], 5, 2); 
			...
			$comicString .= "<td valign='top' id='profilemodcontent'><div align='center'><div class='comictitlelist'>".stripslashes($row['title'])."</div><a href='/create/pdf/?comicid=".$row['comiccrypt']."'>";
...
<form name="PageForm" method="post" action="/create/pdf/?comicid=<? echo $_GET['comicid'];?>">
<input type="text" name="txtUrl" value="" style="width:300px">
<input type="submit" value="GRAB PAGES"></form></div>
<? } else {
	if ((isset($_POST['txtUrl']))&&(isset($_GET['comicid'])) && (!isset($_POST['reorder']))){?>
...
<form name="PageForm" method="post" action="/process/pdf/?comicid=<? echo $_GET['comicid'];?>">
<? if ((isset($_POST['txtUrl']))&&(isset($_GET['comicid'])) && (!isset($_POST['reorder']))){?>
...
<?php
	...
	echo '<ul id="boxes">';
		...
		foreach ($images_url_array as $Image) {
			...
			$gif = @file_get_contents('http://'.trim($Image)) or die('<div style="font-weight:bold; padding:25px;">Sorry we could not find any pages at that address. Please make sure you\'ve entered the correct URL to your Panel Flow Administration install and  try again</div>');
